...
async def HelpWatermark(bot, cmd):
	...
	await cmd.reply_text(
		...
		parse_mode="Markdown",
		reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Developer", url="https://t.me/SupremeYoriichi"), InlineKeyboardButton("Support Channel", url="https://t.me/SharkToonsIndia")], [InlineKeyboardButton("Owner V2", url="https://t.me/OfficialCreater")], [InlineKeyboardButton("Support Git-ID", url="https://github.com/Harshit-Bit10")]]),
...
async def VidWatermarkAdder(bot, cmd):
	...
	if (int(file_size) > 2097152000) and (Config.ALLOW_UPLOAD_TO_STREAMTAPE is True) and (Config.STREAMTAPE_API_USERNAME != "NoNeed") and (Config.STREAMTAPE_API_PASS != "NoNeed"):
		...
		try:
			...
			print(f"Error: {e}")
			await editable.edit("Sorry, Something went wrong!\n\nCan't Upload to Streamtape. You can report at [Support Channel](https://t.me/SharkToonsIndia).")
...
async def button(bot, cmd: CallbackQuery):
	...
	if "refreshmeh" in cb_data:
		...
		await cmd.message.edit(
			text=Config.USAGE_WATERMARK_ADDER,
			parse_mode="Markdown",
			reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Developer", url="https://t.me/SupremeYoriichi"), InlineKeyboardButton("Support Channel", url="https://t.me/SharkToonsIndia")], [InlineKeyboardButton("Git-ID Support", url="https://github.com/harshit-bit10")]]),
			disable_web_page_preview=True
		)
		...
		if Config.UPDATES_CHANNEL:
			...
			try:
				...
				await cmd.message.edit(
					text="Something went Wrong. Contact my [SupportChannel](https://t.me/SharkToonsIndia).",
					parse_mode="markdown",
					disable_web_page_preview=True
				)
				return
